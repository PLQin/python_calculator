...
class Calculator():
	...
	def do_sum(self):
		...
		if self.op == "add":
			...
			# 对数
			self.total = log(self.total, self.current)
		elif self.op == "log":
			# 10的对数
...
if __name__ == '__main__':
	...
	for j in range(1,4):
		for k in range(3):
			btn = Button(calc, height=2, width=4, padx=10, pady=10, text=numbers[i])
			btns.append(btn)
			btns[i]["bg"] = "orange"
			btns[i].grid(row=j, column=k, padx=1, pady=1)
			btns[i]["command"] = lambda x = numbers[i]: sum1.num_press(x)
			i += 1
